...
import client from "./paypalClient"; // PayPal SDK client
  ...
  request.requestBody({
    ...
    purchase_units: [{
      ...
      items: [{
        ...
        unit_amount: {
          currency_code: "USD",
          value: dollarAmount, // Price per single unit (total purchase here)
        },
        quantity: "1",
...
app.post("/capture-payment", async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (captureData.status === "COMPLETED") {
      ...
      try {
        ...
        return res.status(500).json({
          error:
            "Payment successful, but credit update failed. " +
    ...
    const instrumentErrorDetails = err.result?.details?.[0];
    if (err.statusCode === 422 &&
        instrumentErrorDetails?.issue === "INSTRUMENT_DECLINED") {
      functions.logger.warn(
        `Instrument declined for PayPal order ${orderID}.`,
    ...
    const statusCode = err.statusCode || 500;
    const errorMessage =
      instrumentErrorDetails?.description ||
      err.message ||
...
export const helloWorld = functions.https.onRequest((_req, response) => {
