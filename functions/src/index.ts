...
app.post("/create-order", async (req, res) => {
  ...
  if (
    ...
    functions.logger.error(
      "[PayPal API] Invalid input for /create-order:",
      req.body
    );
    ...
    purchase_units: [
      {
        amount: {
          currency_code: "USD",
          value: dollarAmount, // e.g., "1.00" for $1.00
          breakdown: {
            item_total: {
              currency_code: "USD",
              value: dollarAmount,
  ...
  try {
    ...
    functions.logger.info("[PayPal API] Order created successfully:", {
      orderId: order.result.id,
    });
    return res.status(201).json({orderID: order.result.id});
  } catch (err: any) {
    functions.logger.error("[PayPal API] Error creating order:", {
      error: err.message,
...
app.post("/capture-payment", async (req, res) => {
  ...
  if (!orderID || !creditsToPurchase || !userUID) {
    functions.logger.error(
      "[PayPal API] Invalid input for /capture-payment:",
      req.body
    );
    return res.status(400).json({error: "Invalid input parameters."});
  }
  ...
  try {
    ...
    functions.logger.info("[PayPal API] Payment captured successfully:", {
    ...
    if (captureData.status === "COMPLETED") {
      ...
      try {
        ...
        functions.logger.error(
          "[Firestore] Error updating credits for user " +
            `${userUID} after PayPal capture ${captureData.id}:`,
          dbError
        );
        // Payment captured, but DB update failed. Critical error.
      ...
      // Handle other capture statuses (e.g., PENDING, VOIDED)
      functions.logger.warn(
        "[PayPal API] Payment capture for order " +
          `${orderID} status: ${captureData.status}.`
      );
...
export const helloWorld = functions.https.onRequest((request, response) => {
  functions.logger.info("Hello logs!"); // Simplified log
